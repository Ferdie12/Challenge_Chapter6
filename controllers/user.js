...
module.exports = {
    register: async (req, res) => {
        try {
            ...
            const html = await nodemailer.getHtml('activation.ejs', {user: {name: user.name}, url});
            nodemailer.sendMail(user.email, 'Activation Account', html);

            return res.status(201).json({
                status: true,
                message: 'Check your email for activation your account if the email does not go to the main page try checking in the spam folder'
            });
        } catch (error) {
            throw error;
        }
